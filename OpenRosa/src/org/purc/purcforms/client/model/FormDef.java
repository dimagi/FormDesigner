...
public class FormDef implements Serializable{
	 ...
	 */
	public void addPage(GroupDef pageDef){
		if(pages == null)
			pages = new Vector();
	...
	public void addPage(){
		...
		//pages.add(new GroupDef("Page"+pageno,pageno,this));
	}
	 ...
	 */
	public GroupDef setPageName(String name){
		GroupDef pageDef = ((GroupDef)pages.elementAt(pages.size()-1));
		pageDef.setName(name);
		return pageDef;
	}

	public void setPageLabelNode(Element labelNode){
		((GroupDef)pages.elementAt(pages.size()-1)).setLabelNode(labelNode);
	}

	public void setPageGroupNode(Element groupNode){
		((GroupDef)pages.elementAt(pages.size()-1)).setGroupNode(groupNode);
	}
	...
	public GroupDef getPageAt(int index) {
		if(pages == null)
			return null;
		return (GroupDef)pages.elementAt(index);
	}
	...
	public void updateDoc(boolean withData){
		...
		if(pages != null){
			for(int i=0; i<pages.size(); i++){
				GroupDef pageDef = (GroupDef)pages.elementAt(i);
	...
	private void updateDataNodes(){
		...
		for(int i=0; i<pages.size(); i++)
			((GroupDef)pages.elementAt(i)).updateDataNodes(dataNode);
	}
	...
	public QuestionDef getQuestion(int id){		
		...
		for(int i=0; i<getPages().size(); i++){
			QuestionDef def = ((GroupDef)getPages().elementAt(i)).getQuestion(id);
			if(def != null)
				return def;
		}
	...
	public void addQuestion(QuestionDef qtn){
		if(pages == null){
			pages = new Vector();
			//GroupDef page = new GroupDef(/*this.getVariableName()*/"Page1",Integer.parseInt("1"),null,this);
			//pages.addElement(page);
		}

		((GroupDef)pages.elementAt(pages.size()-1)).addQuestion(qtn);
	...
	private void copyPages(Vector pages){
		if(pages != null){
			...
			for(int i=0; i<pages.size(); i++) //Should have atleast one page is why we are not checking for nulls.
				this.pages.addElement(new GroupDef((GroupDef)pages.elementAt(i),this));
		}
	}
	 ...
	 */
	public void removePage(GroupDef pageDef){
		/*for(int i=0; i<pages.size(); i++){
	 ...
	 */
	public void movePageUp(GroupDef pageDef){
		...
		GroupDef currentPageDef;
		List<GroupDef> list = new ArrayList<GroupDef>();

		while(pages.size() >= index){
			currentPageDef = (GroupDef)pages.elementAt(index-1);
		...
		for(int i=0; i<list.size(); i++){
			if(i == 0){
				GroupDef pgDef = (GroupDef)list.get(i);
	 ...
	 * @param pageDef the page to move.
	 */
	public void movePageDown(GroupDef pageDef){
		int index = pages.indexOf(pageDef);	
		...
		GroupDef currentItem; // = parent.getChild(index - 1);
		List<GroupDef> list = new ArrayList<GroupDef>();

		while(pages.size() > 0 && pages.size() > index){
			currentItem = (GroupDef)pages.elementAt(index);
	...
	public boolean removeQuestion(QuestionDef qtnDef){
		for(int i=0; i<pages.size(); i++){
			if(((GroupDef)pages.elementAt(i)).removeQuestion(qtnDef,this))
				return true;
		}
		return false;
	}
	...
	public void moveQuestion2Page(QuestionDef qtn, int pageNo, FormDef formDef){
		if(pages.size() < pageNo)
			pages.add(new GroupDef(formDef));

		for(int i=0; i<pages.size(); i++){
			GroupDef page = (GroupDef)pages.elementAt(i);
			if(page.contains(qtn)){
				if(i == pageNo-1)
					return;
				page.getQuestions().removeElement(qtn);
				((GroupDef)pages.elementAt(pageNo-1)).addQuestion(qtn);
				return;
			}
		}
	}
	...
	public QuestionDef getQuestionWithText(String text){
		for(int i=0; i<pages.size(); i++){
			QuestionDef questionDef = ((GroupDef)pages.elementAt(i)).getQuestionWithText(text);
			if(questionDef != null)
				return questionDef;
		}
		return null;
	...
	public void refresh(FormDef formDef){
		...
		for(int index = 0; index < formDef.getPageCount(); index++)
			refresh((GroupDef)formDef.getPageAt(index));
	...
	private void refresh(GroupDef pageDef){
		for(int index = 0; index < pages.size(); index++)
			((GroupDef)pages.get(index)).refresh(pageDef);
	}
	...
	public Element getLanguageNode() {
		...
		if(dataNode != null){
			...
			if(pages != null){
				for(int index = 0; index < pages.size(); index++)
					((GroupDef)pages.elementAt(index)).buildLanguageNodes(doc, rootNode);
			}
	...
	public static FormDef getFormDef(Object formItem){
		if(formItem instanceof FormDef)
			...
			return ((GroupDef)formItem).getParent();
	...
	public void clearChangeListeners(){
		...
		for(int i=0; i<pages.size(); i++)
			((GroupDef)pages.elementAt(i)).clearChangeListeners();
	}
}
