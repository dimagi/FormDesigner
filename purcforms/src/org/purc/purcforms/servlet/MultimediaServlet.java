	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			while (items.hasNext()) {
				...
				byte[] postData = IOUtils.toByteArray(item.openStream());
				response.getOutputStream().print(Base64.encode(postData));

				setSessionData(request,formId,KEY_MULTIMEDIA_POST_CONTENT_TYPE+getFieldKey(formId,xpath),item.getContentType());
